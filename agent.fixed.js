  ...
  for (const motor of abnormals) {
    try {
      ...
      switch (motor.motorType) {
        case '01': await executeCommand({ action: 'closeGate' }); break;
        case '02': await executeCommand({ action: 'transferStop' }); break;
        case '03': await executeCommand({ action: 'customMotor', params: { motorId: '03', type: '00' } }); break;
        case '04': await executeCommand({ action: 'compactorStop' }); break;
        case '05':
...
async function executeCommand(commandData) {
  ...
  try {
    ...
    if (['takePhoto', 'getWeight'].includes(action)) {
      await delay(action === 'takePhoto' ? 2000 : 1000);
    }
